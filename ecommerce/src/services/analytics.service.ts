...
export async function getDashboardSummary() {
  const [revenueResult, deliveredCount, pendingCount, cancelledCount] = await Promise.all([
    db.order.aggregate({
  ...
  return {
    totalRevenue: (revenueResult._sum.totalAmount ?? new Prisma.Decimal(0)).toFixed(2),
    deliveredCount,
    pendingCount,
    cancelledCount,
  };
...
export async function getOrdersByStatus(): Promise<Record<OrderStatusValue, number>> {
...
export async function getRevenueOverTime(granularity: RevenueGranularity = "day") {
  ...
  return rows.map((r) => ({ date: r.bucket.toISOString(), revenue: r.revenue.toFixed(2) }));
}
